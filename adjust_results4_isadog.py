...
def adjust_results4_isadog(results_dic, dogfile):
    ...
    """           
    dognames_dic = {}
    # Reads in dog names from the file, one name per line, and automatically closes the file
    with open(dogfile, "r") as infile:
        # Process each line in the file until reaching EOF (end-of-file) by 
        # processing the line and adding dog names to dognames_dic using a while loop
        line = infile.readline()
        while line != "":
            # Process line by stripping newline from line
            line = line.strip()
            # Adds dog name to dognames_dic if it doesn't already exist 
